...
public class HBaseConnect {
	public static void main(String[] args) throws IOException {
		 ...
		 * arrays. In the // below, we are converting the String "myLittleRow"
		 * into a byte array to // use as a row key for our update. Once you
		 * have a Put instance, you can // adorn it by setting the names of
		 * columns you want to update on the row, // the timestamp to use in
		 * your update, etc.If no timestamp, the server // applies current time
		 * to the edits. Put p = new Put(Bytes.toBytes("myLittleRow"));
		 * 
		 * // To set the value you'd like to update in the row 'myLittleRow',
		 * specify // the column family, column qualifier, and value of the
		 * table cell you'd // like to update. The column family must already
		 * exist in your table // schema. The qualifier can be anything. All
		 * must be specified as byte // arrays as hbase is all about byte
		 ...
		 * you made into // hbase) table.put(p);
		 * 
		 ...
		 * Get g = new Get(Bytes.toBytes("myLittleRow")); Result r =
		 * table.get(g); byte [] value =
		 * r.getValue(Bytes.toBytes("myLittleFamily"),
		 * Bytes.toBytes("someQualifier")); // If we convert the value bytes, we
		 * should get back 'Some Value', the // value we inserted at this
		 * location. String valueStr = Bytes.toString(value);
		 * System.out.println("GET: " + valueStr);
		 * 
		 * // Sometimes, you won't know the row you're looking for. In this
		 * case, you // use a Scanner. This will give you cursor-like interface
		 * to the contents // of the table. To set up a Scanner, do like you did
		 * above making a Put // and a Get, create a Scan. Adorn it with column
		 * names, etc. Scan s = new Scan();
		 * s.addColumn(Bytes.toBytes("myLittleFamily"),
		 * Bytes.toBytes("someQualifier")); ResultScanner scanner =
		 * table.getScanner(s); try { // Scanners return Result instances. //
		 * Now, for the actual iteration. One way is to use a while loop like
		 * so: for (Result rr = scanner.next(); rr != null; rr = scanner.next())
