...
public class BulkLoad {
	...
	/**
	 * Estimate of imbalance of data distributed across reducers 1.0 = data
	 * equally distributed across reducers
	 */
	...
	/*
	 * public static void main(String[] args) { try { Path input = new
	 * Path(args[0]); String outputPath = args[2]; Path resourceIds = new
	 * Path(outputPath+TripleToResource.RESOURCE_IDS_DIR); Path
	 * convertedTripletsPath = new
	 * Path(outputPath+ResourceToTriple.TEMP_TRIPLETS_DIR);
	 * 
	 * Path string2IdInput = new
	 * Path(outputPath+TripleToResource.ID2STRING_DIR); Path string2IdOutput =
	 * new Path(outputPath+StringIdAssoc.STRING2ID_DIR);
	 * 
	 ...
	 * Integer.parseInt(args[1])); j1.getConfiguration().set("outputPath",
	 * outputPath); j1.waitForCompletion(true);
	 * 
	 * long firstJob = System.currentTimeMillis()-start;
	 * System.out.println("First pass finished in "+firstJob+" ms");
	 * 
	 * //retrieve counter values and build a SortedMap retrieveCounters(j1);
	 * long startPartition =
	 * HBPrefixMatchSchema.updateLastCounter(tripleToResourceReduceTasks,
	 * con.getConfiguration())+1; } catch (Exception e) { e.printStackTrace(); }
	 * }
	 */
	...
	public static void main(String[] args) {
		try {
			if (args.length != 5) {
				System.out
						.println("Usage: bulkLoad <inputPath> <inputSizeEstimate in MB> <outputPath> <schemaSuffix> <onlyTriples(true/false)>");
				return;
			}
			...
			prefMatchSchema.setTableSplitInfo(totalStringCount, numericalCount, tripleToResourceReduceTasks,
					startPartition, onlyTriples);
	...
	private static void bulkLoadQuadTables(Path convertedTripletsPath, LoadIncrementalHFiles bulkLoad)
		...
		// SPOC--------------------
		Job j5 = createPrefixMatchJob(con, convertedTripletsPath, spocPath, HBPrefixMatchSchema.SPOC,
				PrefixMatch.PrefixMatchSPOCMapper.class);
		...
		// OSPC---------------------------
		Job j10 = createPrefixMatchJob(con, convertedTripletsPath, ospcPath, HBPrefixMatchSchema.OSPC,
				PrefixMatch.PrefixMatchOSPCMapper.class);
		j10.waitForCompletion(true);

		doBulkLoad(bulkLoad, ospcPath, currentTable, con);

		// POCS---------------------
		Job j6 = createPrefixMatchJob(con, convertedTripletsPath, pocsPath, HBPrefixMatchSchema.POCS,
				PrefixMatch.PrefixMatchPOCSMapper.class);
		j6.waitForCompletion(true);

		doBulkLoad(bulkLoad, pocsPath, currentTable, con);
	}

	private static void bulkLoadQuadOnlyTables(Path convertedTripletsPath, LoadIncrementalHFiles bulkLoad)
			throws Exception, IOException, InterruptedException, ClassNotFoundException {
		...
		// CSPO----------------------
		Job j7 = createPrefixMatchJob(con, convertedTripletsPath, cspoPath, HBPrefixMatchSchema.CSPO,
				PrefixMatch.PrefixMatchCSPOMapper.class);
		j7.waitForCompletion(true);

		doBulkLoad(bulkLoad, cspoPath, currentTable, con);

		// CPSO-------------------------
		Job j8 = createPrefixMatchJob(con, convertedTripletsPath, cpsoPath, HBPrefixMatchSchema.CPSO,
				PrefixMatch.PrefixMatchCPSOMapper.class);
		j8.waitForCompletion(true);

		doBulkLoad(bulkLoad, cpsoPath, currentTable, con);

		// OCSP--------------------------
		Job j9 = createPrefixMatchJob(con, convertedTripletsPath, ocspPath, HBPrefixMatchSchema.OCSP,
				PrefixMatch.PrefixMatchOCSPMapper.class);
		j9.waitForCompletion(true);

		doBulkLoad(bulkLoad, ocspPath, currentTable, con);
	}

	final private static void bulkLoadIdStringMappingTables(Path idStringAssocInput, LoadIncrementalHFiles bulkLoad)
			throws Exception, IOException, InterruptedException, ClassNotFoundException {
	...
	final private static long runIdGenerationJobs(Path convertedTripletsPath, Path idStringAssocInput)
		...
		long startPartition = HBPrefixMatchSchema.updateLastCounter(tripleToResourceReduceTasks,
				con.getConfiguration(), schemaSuffix) + 1;
		// long startPartition =
		// HBPrefixMatchSchema.getLastCounter(con.getConfiguration())+1;
	...
	private static void runTripleToResourceJob(Path idStringAssocInput, Path resourceIds) throws IOException,
	...
	public static Job createTripleToResourceJob(Path input, Path output, int inputSizeEstimate) throws IOException {
		...
		// for large data we will spill anyway, so might as well start it as
		// soon as possible,
		...
		// tripleToResourceReduceTasks =
		// (int)(Math.ceil((double)totalNumberOfElements/Math.pow(2.0, 24.0)) *
		// LOAD_BALANCER_FACTOR);
	...
	public static Job createResourceToTripleJob(Path input, Path output) throws IOException {
		...
		int reduceTasks = (int) (1.75 * CLUSTER_SIZE * TASK_PER_NODE);
	...
	public static Job createString2IdJob(HBaseConnection con, Path input, Path output) throws Exception {
		...
		conf.setInt("mapred.job.reuse.jvm.num.tasks", -1);
		conf.setFloat("io.sort.record.percent", 0.6f);// intermediate keys are
														// usually bigger than
														// intermediate values
		conf.setFloat("io.sort.spill.percent", 0.6f);
		Job j = new Job(conf);
	...
	public static Job createPrefixMatchJob(HBaseConnection con, Path input, Path output, int tableIndex,
			Class<? extends Mapper> cls) throws Exception {
		Configuration conf = new Configuration();
	...
	public static void retrieveTripleToResourceCounters(Job j1) throws IOException {
		...
		// build histogram
		/*
		 * CounterGroup histogramGroup =
		 * counters.getGroup(TripleToResource.TripleToResourceReducer
		 * .HISTOGRAM_GROUP); Iterator<Counter> it = histogramGroup.iterator();
		 * while (it.hasNext()){ Counter c = it.next(); short b =
		 * Short.parseShort(c.getName(), 16); //System.out.println((char)b
		 * +" "+c.getName()+" "+c.getValue());
		 * 
		 * long half = c.getValue()/2; sufixCounters.put(b, half);
	...
	public static void buildCountersFromFile() throws IOException {
		...
		/*
		 * String line; long totalCounter = 0; while ((line=reader.readLine())
		 * != null){ String []tokens = line.split("@"); short s =
		 * Short.parseShort(tokens[0]); long current =
		 * Long.parseLong(tokens[1]);
		 * System.out.println("Short: "+s+"; Character: "+(char)s+"; "+current);
		 * sufixCounters.put(s, current); totalCounter += current; }
		 * System.out.println(totalCounter);
		 */
	}

	private static void doBulkLoad(LoadIncrementalHFiles bulkLoad, Path dir, HTableInterface table,
			NativeJavaConnection con) throws InterruptedException {
		...
		try {
			...
			System.out
					.println("Bulk load taking longer than usual: sleeping for 10 minutes before proceeding to the next table..");
			Thread.sleep(600000);
			timedOut = true;
		}

		if (timedOut) {// the BulkLoad took too long - which means the cluster
						// is unbalanced
			try {
				con.getAdmin().split(table.getTableName());
				Thread.sleep(300000);// TODO find a better method to do these
										// splits
			} catch (IOException e) {
				e.printStackTrace();
			}
