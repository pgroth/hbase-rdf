	...
	public void populateTables(ArrayList<Statement> statements) throws Exception {
		for (Iterator<Statement> iter = statements.iterator(); iter.hasNext();) {
			...
			if (s.getObject() instanceof Resource){
				addRow(HBasePredicateCFSchema.TABLE_NAME, s.getSubject().toString(), s.getPredicate().toString(), "", s.getObject().toString());
			}
			else {
				addRow(HBasePredicateCFSchema.TABLE_NAME, s.getSubject().toString(), "literal", s.getPredicate().toString(), s.getObject().toString());
			}
		}
	}
