...
public class HBHexastoreUtil implements IHBaseUtil {
	...
	private void computeKeyAndValuePositions(int index, ArrayList<Integer> keyPosition, ArrayList<Integer> valuePosition){
		//the elements from the triple S-P-O that make up the key correspond to the 1 bits in the index parameter
	...
	private void computeKeyAndValuePositions(){
		for (int i = 0; i < HBHexastoreSchema.TABLE_COUNT; i++) {
			...
			logger.debug("Table: "+HBHexastoreSchema.TABLE_NAMES[i]+"Index: "+Integer.toString(i+1)+"; BitCount: "+Integer.toString(bitCount));
	...
	/**
	 * Generate an HBase key from a triple and a table index
	 * see HBHexastoreSchema for the associations between indexes and tables
	 * @param triple
	 * @param index
	 * @return
	 */
	private byte[] getKey(String []triple, int index){
		...
		switch(hashes.length){
			case 3: key = Bytes.add(Bytes.toBytes(hashes[2]), Bytes.toBytes(hashes[1]), Bytes.toBytes(hashes[0])); break;
			case 2: key = Bytes.add(Bytes.toBytes(hashes[1]), Bytes.toBytes(hashes[0])); break;
			case 1: key = Bytes.toBytes(hashes[0]);break;
			default: throw new RuntimeException("Unexpected number of bits");
		}
	...
	/**
	 * Generate an HBase cell value from a triple and a table index
	 * see HBHexastoreSchema for the associations between indexes and tables
	 * @param triple
	 * @param index
	 * @return
	...
	/**
	 * @param tables
	 * @param index - should be between 1 and 7
	 * @param triple
	 * @param batchPut
	 * @throws IOException
	 */
	private boolean addTriple(HTable []tables, int index, String []triple, ArrayList<Put> batchPut) throws IOException{
		...
		logger.debug("Table: "+HBHexastoreSchema.TABLE_NAMES[index]+"; Key: "+new BigInteger(key).abs().toString(16));
		...
		byte []val;
		byte []existingValue = r.getValue(HBHexastoreSchema.COLUMN_FAMILY.getBytes(), HBHexastoreSchema.COLUMN_NAME.getBytes());
	...
	public void populateTables(ArrayList<Statement> statements)
		...
		for (int i=0; i<statements.size(); i++) {
			...
			/*if ((i+1) % FLUSH_LIMIT == 0){
				for (int j = 0; j < tables.length; j++) {
					tables[j].put(batchPuts.get(j));
					batchPuts.get(j).clear();
				}
			}*/
		}
