...
import static com.hp.hpl.jena.sparql.engine.optimizer.reorder.PatternElements.isSet;
...
import nl.vu.jena.sparql.engine.optimizer.Pattern;
...
//    }
    ...
    {        
        ...
        if ( pred.isNodeURI() )
        {
            ...
            return orderedSearchForCandidatePatterns(subj, pred, obj, candidates);
        }
	...
	public double orderedSearchForCandidatePatterns(Item subj, Item pred, Item obj, Item[] candidates) {
		double w = NOMATCH ;
		for (Item item : candidates) {
			w = search(item, subj, pred, obj, w) ;
			if (w!=NOMATCH){
				return w;
			}
		}
